...
export default function Programmers() {
  ...
  const getNameOfFeatured = () => {
    ...
    programmers.forEach(each => {
      each.id === current && (displayName = each.name)
    })
    return displayName;
  };
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: current ? "gold" : 'royalblue' // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          programmers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { setCurrent(dev.id) }}>Feature</button>
            </div>
          )
        }
      </div>
